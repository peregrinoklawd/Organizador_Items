...
if (_orchestrator getOrDefault ["running",false]) exitWith {[false,"ITEMS_0_11_CP_B61_ALREADY_RUNNING","Outra suíte cumulativa já está em execução.",createHashMap] call ServoPeregrino_Organizador_Nexus_fnc_createResult};

private _startedAt=diag_tickTime;
diag_log "============================================================";
diag_log "[SP_ORG] [ITEMS] 0.11 CP-B.6.1 — FOCUSED EQUIPMENT VIEW + CATALOG PERFORMANCE — 436 GATES";
diag_log "============================================================";
...
if !(_baseOk) exitWith {
    [] call ServoPeregrino_Organizador_Items_fnc_prepareCumulativeTestRun;
    private _summary=createHashMapFromArray [["delivery","0.11 CP-B.6.1"],["build",SERVO_PEREGRINO_ORGANIZADOR_ITEMS_BUILD],["success",false],["code","ITEMS_0_11_CP_B61_BASE_428_FAILED"],["base",_base],["checkpointSkipped",true],["checkpointFirstGate",429],["checkpointLastGate",436],["checkpointTotal",0],["checkpointPassed",0],["checkpointFailed",0],["cumulativeExpected",436]];
    missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_TEST_RESULT_VAR,_summary];
    diag_log "[SP_ORG] [ITEMS] 0.11 CP-B.6.1 — FAIL-FAST: baseline CP-B.5 428/428 não fechou.";
    if (hasInterface) then {hint "SP_ORG_Items 0.11 CP-B.6.1 reprovado na baseline 428/428. Consulte o primeiro FAIL no RPT.";};
    _summary
};

missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_TEST_ORCHESTRATOR_VAR,createHashMapFromArray [["running",true],["startedAtTick",_startedAt],["delivery","0.11 CP-B.6.1"]]];

// CP-B.6.1 — sincronização determinística do cache CONFIG_ALL.
// A baseline CP-B.5 pode deixar um getCatalog agendado pelo smoke de onInterfaceLoad.
// Não iniciamos uma segunda varredura concorrente: aguardamos a existente e só fazemos build on-demand se necessário.
private _catalogSyncStartedAt=diag_tickTime;
private _catalogSyncMode="CACHE_READY";
private _catalogStatusR=[] call ServoPeregrino_Organizador_Items_fnc_getCatalogStatus;
private _catalogStatusD=_catalogStatusR getOrDefault ["data",createHashMap];
if !(_catalogStatusD getOrDefault ["cacheBuilt",false]) then {
    if (_catalogStatusD getOrDefault ["buildInProgress",false]) then {
        _catalogSyncMode="WAIT_EXISTING_BUILD";
        private _deadline=diag_tickTime+120;
        waitUntil {
            uiSleep 0.05;
            private _sr=[] call ServoPeregrino_Organizador_Items_fnc_getCatalogStatus;
            private _sd=_sr getOrDefault ["data",createHashMap];
            (_sd getOrDefault ["cacheBuilt",false]) || {!(_sd getOrDefault ["buildInProgress",false])} || {diag_tickTime>=_deadline}
        };
    };
    _catalogStatusR=[] call ServoPeregrino_Organizador_Items_fnc_getCatalogStatus;
    _catalogStatusD=_catalogStatusR getOrDefault ["data",createHashMap];
    if !(_catalogStatusD getOrDefault ["cacheBuilt",false]) then {
        _catalogSyncMode=if (_catalogSyncMode isEqualTo "WAIT_EXISTING_BUILD") then {"WAIT_THEN_BUILD_ON_DEMAND"} else {"BUILD_ON_DEMAND"};
        private _catalogBuildR=[] call ServoPeregrino_Organizador_Items_fnc_getCatalog;
        _catalogStatusR=[] call ServoPeregrino_Organizador_Items_fnc_getCatalogStatus;
        _catalogStatusD=_catalogStatusR getOrDefault ["data",createHashMap];
    };
};
private _catalogReady=(_catalogStatusD getOrDefault ["cacheBuilt",false]) && {(_catalogStatusD getOrDefault ["itemCount",0])>0};
diag_log format ["[SP_ORG] [ITEMS] [TEST_SYNC] checkpoint=CP-B.6.1 mode=%1 ready=%2 items=%3 buildInProgress=%4 waitMs=%5",_catalogSyncMode,_catalogReady,_catalogStatusD getOrDefault ["itemCount",0],_catalogStatusD getOrDefault ["buildInProgress",false],round ((diag_tickTime-_catalogSyncStartedAt)*1000)];
private _passed=0;
private _failed=0;
private _results=[];
private _assert={
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.11 CP-B.6.1] [%1] %2 — %3",_status,_id,_detail];
};
...
private _gate432=_catalogReady && {(_ui432b getOrDefault ["fullRefreshCount",-1]) isEqualTo _full432a} && {(_ui432b getOrDefault ["equipmentCaptureCount",-1]) isEqualTo _cap432a} && {(_ui432b getOrDefault ["catalogFocusedRefreshCount",-1]) isEqualTo (_catFocus432a+1)} && {(_ui432b getOrDefault ["lastRefreshMode",""]) isEqualTo "CATALOG_FOCUSED"};
...
private _gate434=_catalogReady && {(_w434a getOrDefault ["success",false])} && {_w434b getOrDefault ["success",false]} && {_d434a getOrDefault ["projectionBuiltNow",false]} && {!(_d434b getOrDefault ["projectionBuiltNow",true])} && {(_d434a getOrDefault ["projectionBuildCount",0]) isEqualTo (_d434b getOrDefault ["projectionBuildCount",-1])} && {(_d434b getOrDefault ["defensiveCopies",9999]) <= 10};
...
private _gate435=_catalogReady && {(_ui435b getOrDefault ["fullRefreshCount",-1]) isEqualTo _full435a} && {(_ui435b getOrDefault ["equipmentCaptureCount",-1]) isEqualTo _cap435a} && {(_ui435b getOrDefault ["catalogFocusedRefreshCount",-1]) isEqualTo (_catFocus435a+2)};
...
private _summary=createHashMapFromArray [["delivery","0.11 CP-B.6.1"],["build",SERVO_PEREGRINO_ORGANIZADOR_ITEMS_BUILD],["success",_failed isEqualTo 0],["base",_base],["baseExpected",428],["checkpointFirstGate",429],["checkpointLastGate",436],["checkpointTotal",_passed+_failed],["checkpointPassed",_passed],["checkpointFailed",_failed],["checkpointResults",_results],["cumulativeExpected",436],["cumulativePassed",428+_passed],["durationMs",_durationMs],["focusedEquipmentView",true],["focusedCatalog",true],["audioEnabled",true],["catalogReadyBeforeFocusedGates",_catalogReady],["catalogSyncMode",_catalogSyncMode]];
missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_TEST_RESULT_VAR,_summary];
diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] 0.11 CP-B.6.1 — BASE=428/428  CP-B.6.1=%1/8  FAIL=%2  CUMULATIVO=%3/436  TEMPO=%4ms",_passed,_failed,428+_passed,_durationMs];
diag_log "[SP_ORG] [ITEMS] 0.11 CP-B.6.1 — Esperado manual: U→U sem recaptura; U/C/M com EQUIPMENT_FOCUSED; filtros com CATALOG_FOCUSED; depois do Catálogo pronto, trocar kits e procurar UI_PERF_KIT_SWITCH.";
diag_log "============================================================";
if (hasInterface) then {hint parseText format ["<t size='1.25'>SP_ORG_Items 0.11 CP-B.6.1</t><br/><br/>Baseline CP-B.5: <t color='#7CFC00'>428 / 428</t><br/>CP-B.6.1: <t color='%1'>%2 / 8</t><br/>Cumulativo: %3 / 436<br/><br/>%4",if (_failed isEqualTo 0) then {"#7CFC00"} else {"#FF6B6B"},_passed,428+_passed,if (_failed isEqualTo 0) then {"Teste VIEW U/C/M, filtros e troca de kits com Catálogo pronto; envie UI_PERF/UI_PERF_KIT_SWITCH."} else {"Consulte o primeiro FAIL no RPT."}];};
_summary
