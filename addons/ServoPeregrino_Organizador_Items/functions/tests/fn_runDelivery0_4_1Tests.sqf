...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.4.1] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_4_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
["ITEMS-0.4-002", (_build getOrDefault ["displayVersion", ""]) isEqualTo "0.4.1", "displayVersion é 0.4.1."] call _assert;
["ITEMS-0.4-003", (_build getOrDefault ["semanticVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_SEMANTIC_VERSION, "semanticVersion corresponde à candidata 0.4.1."] call _assert;
...
private _clearAgain = ["AUTO_0_4_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
// Fixture físico controlado: mutações abaixo pertencem somente ao runner de teste.
private _originalLoadout = getUnitLoadout player;
private _originalFingerprint = str _originalLoadout;
...
player addItemToUniform "FirstAidKit";
(uniformContainer player) addMagazineAmmoCargo ["30Rnd_65x39_caseless_mag", 1, 30];
(uniformContainer player) addMagazineAmmoCargo ["30Rnd_65x39_caseless_mag", 1, 17];
(uniformContainer player) addMagazineAmmoCargo ["30Rnd_65x39_caseless_mag", 1, 6];
(vestContainer player) addItemCargo ["H_HelmetB", 1];
(backpackContainer player) addWeaponCargo ["arifle_MX_F", 1];

private _fixtureBefore = [player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint;
private _fixtureBeforeData = _fixtureBefore getOrDefault ["data", createHashMap];
private _resolveU = [player, "U"] call ServoPeregrino_Organizador_Items_fnc_resolvePlayerContainer;
...
private _captureU = [player, "U"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainer;
...
private _fixtureAfterU = [player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint;
...
private _captureAny = [player, "ANY"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainers;
...
private _captureC = [player, "C"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainer;
...
private _captureM = [player, "M"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainer;
...
removeBackpack player;
private _missingM = [player, "M"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainer;
...
private _anyMissingM = [player, "ANY"] call ServoPeregrino_Organizador_Items_fnc_capturePlayerContainers;
...
private _restoredFingerprint = str (getUnitLoadout player);
["ITEMS-0.4-122", _restoredFingerprint isEqualTo _originalFingerprint, "Runner restaura integralmente o loadout original após o fixture."] call _assert;
["ITEMS-0.4.1-123", (_captureUData getOrDefault ["provider", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_CONTAINER_PROVIDER, "capturePlayerContainer devolve provider PLAYER_CONTAINERS sem erro de preprocessor/macro."] call _assert;

// Limpeza obrigatória do namespace automatizado. Produção nunca foi alvo das escritas acima.
private _cleanupTestStorage = ["AUTO_0_4_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
// Restaurar runtime normal de produção após a suíte; não faz parte do gate 123/123.
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
...
if !(_restoreProduction getOrDefault ["success", false]) then {
    diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.4.1 passaram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];
};

diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.4.1 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";

if (hasInterface) then {
    hint parseText format [
        "<t size='1.25'>SP_ORG_Items — Testes 0.4.1</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>123 / 123</t><br/>Consulte o RPT para os IDs individuais.",
        _passed,
