...
if (_existingTestState getOrDefault ["running", false]) exitWith {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.7.2] [BUSY] Suíte já está em execução (%1 ms). Nova execução recusada.", _elapsed];
    if (hasInterface) then {
        hint format ["SP_ORG_Items 0.7.2: suíte já em execução (%1 ms). Aguarde o resumo final.", _elapsed];
    };
...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.7.2] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_7_2", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
["ITEMS-0.4-002", (_build getOrDefault ["displayVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_DISPLAY_VERSION, "displayVersion corresponde ao header da candidata 0.7.2."] call _assert;
["ITEMS-0.4-003", (_build getOrDefault ["semanticVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_SEMANTIC_VERSION, "semanticVersion corresponde ao header da candidata 0.7.2."] call _assert;
...
private _clearAgain = ["AUTO_0_7_2", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
diag_log "[SP_ORG] [ITEMS] [TEST 0.7.2] [INFO] Iniciando o único build completo CONFIG_ALL da suíte cumulativa rápida. Em modsets grandes aguarde os logs BUILD_PROGRESS; não execute a suíte novamente.";
...
private _build07=[] call ServoPeregrino_Organizador_Items_fnc_getBuildInfo; ["ITEMS-0.7-245", (_build07 getOrDefault ["delivery",""]) isEqualTo "0.7.2" && {(_build07 getOrDefault ["uiDisplayIdd",0]) isEqualTo 7700}, "BuildInfo publica entrega 0.7.2 e contrato básico da UI."] call _assert;
...
private _cleanupTestStorage = ["AUTO_0_7_2", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
// Restaurar runtime normal de produção após a suíte; não altera o gate 254/254.
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
...
if !(_restoreProduction getOrDefault ["success", false]) then {
    diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.7.2 concluíram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];
};
// resetRuntimeForTests limpa a variável de resultado; republicar o summary após restaurar produção.
missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_TEST_RESULT_VAR, _summary];
missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_TEST_RUNNING_VAR, createHashMap];

diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.7.2 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";

if (hasInterface) then {
    hint parseText format [
        "<t size='1.25'>SP_ORG_Items — Testes 0.7.2</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>265 / 265</t><br/>Consulte o RPT para os IDs individuais.",
        _passed,
