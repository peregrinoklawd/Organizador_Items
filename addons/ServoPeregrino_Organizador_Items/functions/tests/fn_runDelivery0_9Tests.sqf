...
if (_existingTestState getOrDefault ["running", false]) exitWith {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9] [BUSY] Suíte já está em execução (%1 ms). Nova execução recusada.", _elapsed];
    if (hasInterface) then {
        hint format ["SP_ORG_Items 0.9: suíte já em execução (%1 ms). Aguarde o resumo final.", _elapsed];
    };
...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_9", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _clearAgain = ["AUTO_0_9", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
diag_log "[SP_ORG] [ITEMS] [TEST 0.9] [INFO] Iniciando o único build completo CONFIG_ALL da suíte cumulativa rápida. Em modsets grandes aguarde os logs BUILD_PROGRESS; não execute a suíte novamente.";
...
private _cleanupTestStorage = ["AUTO_0_9", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _summary = createHashMapFromArray [
    ["delivery", "0.9"], ["build", SERVO_PEREGRINO_ORGANIZADOR_ITEMS_BUILD], ["total", _total], ["passed", _passed], ["failed", _failed],
...
diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.9 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";
if (hasInterface) then {hint parseText format ["<t size='1.25'>SP_ORG_Items — Testes 0.9</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>340 / 340</t><br/>Depois execute o gate manual físico isolado 0.9.",_passed,if (_failed isEqualTo 0) then {"#7CFC00"} else {"#FF6B6B"},_failed,_total,_durationMs];};
_summary
