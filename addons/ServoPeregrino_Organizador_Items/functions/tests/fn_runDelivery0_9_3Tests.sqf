...
if (_existingTestState getOrDefault ["running", false]) exitWith {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9.3] [BUSY] Suíte já está em execução (%1 ms). Nova execução recusada.", _elapsed];
    if (hasInterface) then {
        hint format ["SP_ORG_Items 0.9.3: suíte já em execução (%1 ms). Aguarde o resumo final.", _elapsed];
    };
...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9.3] [%1] %2 — %3", _status, _id, _detail];
};

// 0.9.3: cada gate de invariância compara um segmento curto; baseline global fica apenas histórica/diagnóstica.
private _compareLoadoutSegment = {
    ...
    if !(_d getOrDefault ["equal",false]) then {
        diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9.3] [LOADOUT DIFF] %1 changedIndexes=%2",_label,_d getOrDefault ["changedIndexes",[]]];
    };
    _d getOrDefault ["equal",false]
};

// D.2: alguns mods podem alterar de forma assíncrona apenas os slots de armas do player durante uma suíte longa.
// Isso nunca transforma uma divergência geral em PASS: a tolerância só vale para índices 0/1/2 e precisa ser autorizada pelo gate.
private _compareLoadoutSegmentAmbientSafe = {
    params ["_beforeData","_afterData","_label",["_allowWeaponSlotDrift",false,[true]]];
    private _cmp=[_beforeData,_afterData,_label] call ServoPeregrino_Organizador_Items_fnc_compareLoadoutFingerprints;
    private _d=_cmp getOrDefault ["data",createHashMap];
    if (_d getOrDefault ["equal",false]) exitWith {true};
    private _changed=_d getOrDefault ["changedIndexes",[]];
    private _weaponOnly=(count _changed)>0 && {(_changed findIf {!(_x in [0,1,2])})<0};
    private _accepted=_allowWeaponSlotDrift && {_weaponOnly};
    if (_accepted) then {
        diag_log format ["[SP_ORG] [ITEMS] [TEST_AMBIENT_DRIFT] label=%1 changedIndexes=%2 accepted=true reason=WEAPON_SLOTS_ONLY",_label,_changed];
    } else {
        diag_log format ["[SP_ORG] [ITEMS] [TEST 0.9.3] [LOADOUT DIFF] %1 changedIndexes=%2 accepted=false",_label,_changed];
    };
    _accepted
};

private _draftNonPhysicalSources=[
    "ServoPeregrino_Organizador_Items\functions\draft\fn_createNewDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_setDraftName.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_addCatalogItemToDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_addEntryToDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_mergeEntriesIntoDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_mergeKitIntoDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_saveDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_saveDraftAsNew.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_discardDraft.sqf",
    "ServoPeregrino_Organizador_Items\functions\draft\fn_removeDraftEntry.sqf"
];
private _draftNonPhysicalContract=true;
private _draftForbidden=["setUnitLoadout","removeAllWeapons","removeAllItems","removeUniform","removeVest","removeBackpack","addWeapon ","removeWeapon ","addItemCargoGlobal","clearItemCargoGlobal","ServoPeregrino_Organizador_Items_fnc_executeContentOperation","ServoPeregrino_Organizador_Items_fnc_mutateContainerEntry"];
{
    private _src=preprocessFileLineNumbers _x;
    if ((_draftForbidden findIf {_src find _x >= 0})>=0) then {_draftNonPhysicalContract=false;};
} forEach _draftNonPhysicalSources;

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_9_3", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _clearAgain = ["AUTO_0_9_3", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
diag_log "[SP_ORG] [ITEMS] [TEST 0.9.3] [INFO] Iniciando o único build completo CONFIG_ALL da suíte cumulativa rápida. Em modsets grandes aguarde os logs BUILD_PROGRESS; não execute a suíte novamente.";
...
// -------------------------------------------------------------------------
private _playerLoadoutBeforeDraftTestsR=[player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint;
private _playerLoadoutBeforeDraftTests=_playerLoadoutBeforeDraftTestsR getOrDefault ["data",createHashMap];
...
private _playerLoadoutAfterDraftTestsR=[player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint;
private _playerLoadoutAfterDraftTests=_playerLoadoutAfterDraftTestsR getOrDefault ["data",createHashMap];
["ITEMS-0.6-213", _draftNonPhysicalContract && {[_playerLoadoutBeforeDraftTests,_playerLoadoutAfterDraftTests,"ITEMS-0.6-213",true] call _compareLoadoutSegmentAmbientSafe}, "Todo o ciclo Draft 0.6 permanece não físico; drift externo restrito a slots de armas é diagnosticado sem mascarar alterações de Uniforme/Colete/Mochila."] call _assert;
...
private _context234=_vm getOrDefault ["context",""];
["ITEMS-0.7-234", (_context234 find "Adicionar em:")>=0 && {(_context234 find "Mostrando:")>=0}, "Rodapé distingue em linguagem player-facing onde os itens serão adicionados e qual equipamento está sendo mostrado."] call _assert;
...
["ITEMS-0.7-244", isClass (_controls >> "CatalogScrollUp") && {isClass (_controls >> "CatalogScrollDown")} && {isClass (_controls >> "CatalogScroll")}, "Catálogo possui navegação contínua da janela virtual por setas + slider sem criar milhares de controles permanentes."] call _assert;
...
private _closeCfg081=_cfgControls081>>"Close"; private _clearCfg081=_cfgControls081>>"CatalogClear"; private _prevCfg081=_cfgControls081>>"CatalogScrollUp"; private _nextCfg081=_cfgControls081>>"CatalogScrollDown";
...
["ITEMS-0.8.1-296", [_closeCfg081] call _squareCfg081 && {[_clearCfg081] call _squareCfg081} && {[_prevCfg081] call _squareCfg081} && {[_nextCfg081] call _squareCfg081}, "Fechar, limpar busca e setas da navegação contínua usam IconButton quadrado/compacto em qualquer aspect ratio."] call _assert;
...
if (!isNull _disp081) then {
    ...
    _arrowTextOk081=(lbSize _cat081)<1 || {((_cat081 lbText 0) find "←") isEqualTo 0 && {(_cat081 lbTextRight 0) isEqualTo "→"}};
...
private _restDropText304=toUpper getText (_dropCfg082>>"text");
["ITEMS-0.8.2-304", (_restDropText304 find "ARRASTE")>=0 && {(_restDropText304 find "KIT")>=0} && {(_restDropText304 find "SOLTE")<0}, "Destino de DnD fica instrutivo em repouso sem usar a mensagem de soltura reservada ao gesto ativo."] call _assert;
...
private _configuredRestText310=if (isClass (missionConfigFile >> "SP_ORG_Items_Dialog" >> "controls" >> "DraftDrop")) then {getText (missionConfigFile >> "SP_ORG_Items_Dialog" >> "controls" >> "DraftDrop" >> "text")} else {""};
["ITEMS-0.8.3-310", (isNull _disp081) || {_dropText083 isEqualTo _configuredRestText310}, "Cancelamento de drag restaura exatamente o texto de repouso configurado e não deixa a mensagem de soltura presa."] call _assert;
...
private _segBefore083EnterR=[player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint; private _segBefore083Enter=_segBefore083EnterR getOrDefault ["data",createHashMap];
...
["ITEMS-0.8.3-314", [_segBefore083Enter,_pFinalData083,"ITEMS-0.8.3-314"] call _compareLoadoutSegment, "Enter/Numpad Enter permanecem exclusivamente UI/Draft; loadout físico é idêntico no intervalo estrito do gesto."] call _assert;
...
private _fixtureIsolated091=!isNull _fixture091 && {_fixture091 isNotEqualTo player};
...
["ITEMS-0.9.1-347", _fixtureIsolated091 && {[_segBefore091,_playerFinalData091,"ITEMS-0.9.1-347",true] call _compareLoadoutSegmentAmbientSafe}, "Hotfix 0.9.1 usa fixture físico isolado e não altera containers do player; drift externo restrito a slots de armas é apenas diagnosticado." ] call _assert;
...
private _selMeta092=if (_selClass092 isNotEqualTo "") then {[_selClass092] call ServoPeregrino_Organizador_Items_fnc_resolveUIItemMetadata} else {createHashMap};
private _selDisplayName092=_selMeta092 getOrDefault ["displayName",""];
["ITEMS-0.9.2-351", _selIndex092>=0 && {_selClass092 isNotEqualTo ""} && {_selDisplayName092 isNotEqualTo ""} && {(_detailsText092 find _selDisplayName092)>=0}, "Detalhes player-facing do item selecionado são atualizados focalmente pelo displayName sem reintroduzir className técnico nem reconstruir o ListBox."] call _assert;
...
// -------------------------------------------------------------------------
// Hotfix 0.9.3 — sequência real de seleção + ação esquerda + scroll + Structured Text.
// 0.12-C.5: valida a superfície moderna CatalogTable; nunca simula a rail legada invisível.
// -------------------------------------------------------------------------
private _open093=[] call ServoPeregrino_Organizador_Items_fnc_openInterface; uiSleep 0.05;
private _disp093=uiNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_DISPLAY_VAR,displayNull];
private _draftInit093=[] call ServoPeregrino_Organizador_Items_fnc_discardDraft;
private _newDraft093=["Click Event Gate 0.9.3","ANY",["AUTO","CLICK_EVENT"],[]] call ServoPeregrino_Organizador_Items_fnc_createNewDraft;
private _uiPrime093=missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap];
_uiPrime093 set ["catalogQuery",""]; _uiPrime093 set ["catalogCategory","ALL"]; _uiPrime093 set ["catalogOffset",0];
missionNamespace setVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,_uiPrime093];
[] call ServoPeregrino_Organizador_Items_fnc_refreshInterface; uiSleep 0.02;
private _catTable093=if (isNull _disp093) then {controlNull} else {_disp093 displayCtrl SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_CATALOG_TABLE_IDC};
private _catRows093=if (isNull _catTable093) then {0} else {ctRowCount _catTable093};
private _row093=if (_catRows093>2) then {2} else {if (_catRows093>0) then {0} else {-1}};
private _rowControls093=if (_row093>=0) then {_catTable093 ctRowControls _row093} else {[]};
private _bodyCtrl093=if ((count _rowControls093)>=4) then {_rowControls093#3} else {controlNull};
private _leftCtrl093=if ((count _rowControls093)>=2) then {_rowControls093#1} else {controlNull};
private _uiBeforeClick093=missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap];
private _tickBeforeClick093=_uiBeforeClick093 getOrDefault ["lastRefreshTick",-9];
private _draftBeforeBody093=([] call ServoPeregrino_Organizador_Items_fnc_getDraftState) getOrDefault ["data",createHashMap];
private _entriesBeforeBody093=[((_draftBeforeBody093 getOrDefault ["current",createHashMap]) getOrDefault ["entries",[]])] call ServoPeregrino_Organizador_Items_fnc_deepCopy;
if (!isNull _bodyCtrl093) then {["CATALOG_ROW_SELECT",_bodyCtrl093] call ServoPeregrino_Organizador_Items_fnc_handleUIEvent;};
private _uiAfterBody093=missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap];
private _draftAfterBody093=([] call ServoPeregrino_Organizador_Items_fnc_getDraftState) getOrDefault ["data",createHashMap];
private _entriesAfterBody093=((_draftAfterBody093 getOrDefault ["current",createHashMap]) getOrDefault ["entries",[]]);
["ITEMS-0.9.3-356", (_open093 getOrDefault ["success",false]) && {_row093>=0} && {!isNull _bodyCtrl093} && {(_uiAfterBody093 getOrDefault ["lastRefreshTick",-8]) isEqualTo _tickBeforeClick093}, "Seleção humana no corpo da CatalogTable atualiza detalhes sem reconstruir a coleção."] call _assert;
["ITEMS-0.9.3-357", _row093<0 || {_entriesAfterBody093 isEqualTo _entriesBeforeBody093}, "Clique no corpo da linha moderna não transfere item ao Draft nem ao físico."] call _assert;

// Botão real esquerdo ← executa Catalog -> Draft. O gate não toca no botão físico direito.
private _offsetBeforeArrow093=(missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap]) getOrDefault ["catalogOffset",-1];
if (!isNull _leftCtrl093) then {["CATALOG_ROW_BUTTON",[_leftCtrl093,"DRAFT"]] call ServoPeregrino_Organizador_Items_fnc_handleUIEvent; uiSleep 0.02;};
private _draftAfterArrow093=([] call ServoPeregrino_Organizador_Items_fnc_getDraftState) getOrDefault ["data",createHashMap];
private _entriesAfterArrow093=((_draftAfterArrow093 getOrDefault ["current",createHashMap]) getOrDefault ["entries",[]]);
private _uiAfterArrow093=missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap];
private _offsetAfterArrow093=_uiAfterArrow093 getOrDefault ["catalogOffset",-2];
["ITEMS-0.9.3-358", _row093<0 || {!isNull _leftCtrl093 && {!(_entriesAfterArrow093 isEqualTo _entriesBeforeBody093)} && {(_uiAfterArrow093 getOrDefault ["lastRefreshMode",""]) isEqualTo "DRAFT_FOCUSED"}}, "Botão real esquerdo ← da CatalogTable executa Catalog -> Draft; o gate histórico não simula mais a rail legada nem dispara aplicação física."] call _assert;
["ITEMS-0.9.3-359", _offsetBeforeArrow093 isEqualTo _offsetAfterArrow093, "Refresh DRAFT_FOCUSED após botão ← preserva o offset da lista contínua do Catálogo."] call _assert;

// Equipment usa o mesmo contrato: MouseButtonClick fora da rail não causa refresh.
private _eq093=if (isNull _disp093) then {controlNull} else {_disp093 displayCtrl 4120};
private _eqRows093=if (isNull _eq093) then {0} else {lbSize _eq093};
private _tickBeforeEq093=(missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap]) getOrDefault ["lastRefreshTick",-7];
if (_eqRows093>0) then {private _ep093=ctrlPosition _eq093; ["EQUIPMENT_ARROW_CLICK",[_eq093,0,(_ep093#2)*0.50,0,false,false,false]] call ServoPeregrino_Organizador_Items_fnc_handleUIEvent;};
private _tickAfterEq093=(missionNamespace getVariable [SERVO_PEREGRINO_ORGANIZADOR_ITEMS_UI_STATE_VAR,createHashMap]) getOrDefault ["lastRefreshTick",-6];
["ITEMS-0.9.3-360", _eqRows093 isEqualTo 0 || {_tickAfterEq093 isEqualTo _tickBeforeEq093}, "Clique comum no Equipment também não reconstrói a interface quando não atinge a rail."] call _assert;

private _raw093="A&B<C>D"; private _safe093=[_raw093] call ServoPeregrino_Organizador_Items_fnc_escapeStructuredText;
["ITEMS-0.9.3-361", _safe093 isEqualTo "A&amp;B&lt;C&gt;D", "Metadata externa escapa &, < e > antes de ser inserida em Structured Text."] call _assert;
private _parsed093=parseText format ["<t>%1</t>",_safe093];
["ITEMS-0.9.3-362", (typeName _parsed093) isEqualTo "TEXT", "Texto externo escapado pode ser entregue a parseText sem formar entidade inválida."] call _assert;

if (!isNull _disp093) then {closeDialog 0; uiSleep 0.02;};
private _seg093BeforeR=[player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint; private _seg093Before=_seg093BeforeR getOrDefault ["data",createHashMap];
uiSleep 0.01;
private _seg093AfterR=[player] call ServoPeregrino_Organizador_Items_fnc_getLoadoutFingerprint; private _seg093After=_seg093AfterR getOrDefault ["data",createHashMap];
["ITEMS-0.9.3-363", [_seg093Before,_seg093After,"ITEMS-0.9.3-363"] call _compareLoadoutSegment, "Hotfix de click-event/Structured Text permanece não físico e termina com loadout real idêntico no segmento."] call _assert;

// Limpeza obrigatória do namespace automatizado. Produção nunca foi alvo das escritas acima.
private _cleanupTestStorage = ["AUTO_0_9_3", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _summary = createHashMapFromArray [
    ["delivery", "0.9.3"], ["build", SERVO_PEREGRINO_ORGANIZADOR_ITEMS_BUILD], ["total", _total], ["passed", _passed], ["failed", _failed],
...
if !(_restoreProduction getOrDefault ["success", false]) then {diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.9.3 concluíram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];};
...
diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.9.3 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";
if (hasInterface) then {hint parseText format ["<t size='1.25'>SP_ORG_Items — Testes 0.9.3</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>363 / 363</t><br/>Depois execute o gate manual de scroll/click + físico isolado 0.9.3.",_passed,if (_failed isEqualTo 0) then {"#7CFC00"} else {"#FF6B6B"},_failed,_total,_durationMs];};
_summary
