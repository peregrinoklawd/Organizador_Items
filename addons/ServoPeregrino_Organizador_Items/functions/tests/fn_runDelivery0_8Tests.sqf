...
if (_existingTestState getOrDefault ["running", false]) exitWith {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.8] [BUSY] Suíte já está em execução (%1 ms). Nova execução recusada.", _elapsed];
...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.8] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_8", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
["ITEMS-0.4-002", (_build getOrDefault ["displayVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_DISPLAY_VERSION, "displayVersion corresponde ao header da candidata 0.8."] call _assert;
["ITEMS-0.4-003", (_build getOrDefault ["semanticVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_SEMANTIC_VERSION, "semanticVersion corresponde ao header da candidata 0.8."] call _assert;
...
["ITEMS-0.4-013", (_uiMetadata getOrDefault ["ready", false]) && {(_uiMetadata getOrDefault ["status", ""]) isEqualTo "SHELL_READ_ONLY"}, "items.ui v1 agora publica Four-Panel UI Shell como pronto e read-only."] call _assert;
...
private _clearAgain = ["AUTO_0_8", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
diag_log "[SP_ORG] [ITEMS] [TEST 0.8] [INFO] Iniciando o único build completo CONFIG_ALL da suíte cumulativa rápida. Em modsets grandes aguarde os logs BUILD_PROGRESS; não execute a suíte novamente.";
...
["ITEMS-0.7-220", (_uiStateR getOrDefault ["success",false]) && {(_uiState getOrDefault ["ready",false])} && {(_uiState getOrDefault ["readOnlyShell",false])}, "UI State v1 inicia pronto em modo shell somente leitura."] call _assert;
...
["ITEMS-0.7-226", _vm getOrDefault ["readOnlyShell",false], "View-model declara shell read-only; UI 0.7 não antecipa Application Engine."] call _assert;
...
["ITEMS-0.8-281", (_target08 getOrDefault ["success",false]) && {(_draft08 getOrDefault ["preferredTarget",""]) isEqualTo "VEST"}, "Destino recomendado U/C/M do kit é editável e separado do destino de aplicação."] call _assert;
...
private _cfgText08=preprocessFileLineNumbers "ServoPeregrino_Organizador_Items\ui\items_dialog.hpp";
...
["ITEMS-0.8-289", (_cfgText08 find "onLBDrag")>=0 && {(_cfgText08 find "onLBDrop")>=0} && {(_cfgText08 find "CATALOG_ARROW_CLICK")>=0} && {(_cfgText08 find "POINTER_DOWN")>=0} && {(_pointerResolver289 find "ctRowControls")>=0} && {(_dragHandler289 find "DISPLAY_POINTER")>=0}, "Config runtime contém DnD atual: Kit nativo e Catálogo/Equipment/Draft por ponteiro unificado do display sobre CT_CONTROLS_TABLE."] call _assert;
...
private _cleanupTestStorage = ["AUTO_0_8", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _summary = createHashMapFromArray [
    ["delivery", "0.8"], ["build", SERVO_PEREGRINO_ORGANIZADOR_ITEMS_BUILD], ["total", _total], ["passed", _passed], ["failed", _failed],
...
if !(_restoreProduction getOrDefault ["success", false]) then {diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.8 concluíram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];};
...
diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.8 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";
if (hasInterface) then {hint parseText format ["<t size='1.25'>SP_ORG_Items — Testes 0.8</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>292 / 292</t><br/>Depois execute o gate manual de Draft/UI.",_passed,if (_failed isEqualTo 0) then {"#7CFC00"} else {"#FF6B6B"},_failed,_total,_durationMs];};
_summary
