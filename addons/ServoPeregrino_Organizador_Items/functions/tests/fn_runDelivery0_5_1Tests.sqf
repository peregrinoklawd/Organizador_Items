...
if (_existingTestState getOrDefault ["running", false]) exitWith {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.5.1] [BUSY] Suíte já está em execução (%1 ms). Nova execução recusada.", _elapsed];
    if (hasInterface) then {
        hint format ["SP_ORG_Items 0.5.1: suíte já em execução (%1 ms). Aguarde o resumo final.", _elapsed];
    };
...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.5.1] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_5_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
["ITEMS-0.4-002", (_build getOrDefault ["displayVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_DISPLAY_VERSION, "displayVersion corresponde ao header da candidata 0.5.1."] call _assert;
["ITEMS-0.4-003", (_build getOrDefault ["semanticVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_SEMANTIC_VERSION, "semanticVersion corresponde ao header da candidata 0.5.1."] call _assert;
...
private _clearAgain = ["AUTO_0_5_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
diag_log "[SP_ORG] [ITEMS] [TEST 0.5.1] [INFO] Iniciando primeiro build completo CONFIG_ALL. Em modsets grandes aguarde os logs BUILD_PROGRESS; não execute a suíte novamente.";
...
private _invalidate = ["AUTO_TEST_0_5_1"] call ServoPeregrino_Organizador_Items_fnc_invalidateCatalogCache;
...
["ITEMS-0.5-160", (_statusInvalidatedData getOrDefault ["invalidationCount", 0]) isEqualTo (_invalidateBefore + 1) && {(_statusInvalidatedData getOrDefault ["lastInvalidationReason", ""]) isEqualTo "AUTO_TEST_0_5_1"}, "Invalidação é observável e registra razão sem varrer configs."] call _assert;
...
private _cleanupTestStorage = ["AUTO_0_5_1", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
// Restaurar runtime normal de produção após a suíte; não altera o gate 171/171.
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
...
if !(_restoreProduction getOrDefault ["success", false]) then {
    diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.5.1 concluíram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];
};
...
diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.5.1 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";

if (hasInterface) then {
    hint parseText format [
        "<t size='1.25'>SP_ORG_Items — Testes 0.5.1</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>171 / 171</t><br/>Consulte o RPT para os IDs individuais.",
        _passed,
