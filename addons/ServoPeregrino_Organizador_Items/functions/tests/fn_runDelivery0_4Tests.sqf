...
private _assert = {
    ...
    diag_log format ["[SP_ORG] [ITEMS] [TEST 0.4] [%1] %2 — %3", _status, _id, _detail];
};

[] call ServoPeregrino_Organizador_Nexus_fnc_initialize;
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
private _testModeSetup = ["AUTO_0_4", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
["ITEMS-0.4-002", (_build getOrDefault ["displayVersion", ""]) isEqualTo "0.4", "displayVersion é 0.4."] call _assert;
["ITEMS-0.4-003", (_build getOrDefault ["semanticVersion", ""]) isEqualTo SERVO_PEREGRINO_ORGANIZADOR_ITEMS_SEMANTIC_VERSION, "semanticVersion corresponde à candidata 0.4."] call _assert;
...
private _clearAgain = ["AUTO_0_4", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
private _cleanupTestStorage = ["AUTO_0_4", true] call ServoPeregrino_Organizador_Items_fnc_configureRepositoryTestMode;
...
// Restaurar runtime normal de produção após a suíte; não faz parte do gate 122/122.
[] call ServoPeregrino_Organizador_Items_fnc_resetRuntimeForTests;
...
if !(_restoreProduction getOrDefault ["success", false]) then {
    diag_log format ["[SP_ORG] [ITEMS] [WARN] Testes 0.4 passaram, mas restauração do runtime de produção retornou %1", _restoreProduction getOrDefault ["code", "UNKNOWN"]];
};

diag_log "============================================================";
diag_log format ["[SP_ORG] [ITEMS] ENTREGA 0.4 — TOTAL=%1 PASS=%2 FAIL=%3 TEMPO=%4ms", _total, _passed, _failed, _durationMs];
diag_log "============================================================";

if (hasInterface) then {
    hint parseText format [
        "<t size='1.25'>SP_ORG_Items — Testes 0.4</t><br/><br/><t color='#7CFC00'>PASS: %1</t><br/><t color='%2'>FAIL: %3</t><br/>Total: %4<br/>Tempo: %5 ms<br/><br/>Gate esperado: <t color='#7CFC00'>122 / 122</t><br/>Consulte o RPT para os IDs individuais.",
        _passed,
